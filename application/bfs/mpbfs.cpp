...
int main(int argc, char** argv)
{
	...
	if (sched_setaffinity(0, sizeof(mask), &mask) == -1) {
                cerr << "could not set CPU affinity in process " << processid << endl;
        }

	char filename[32];
	sprintf(filename, "%s_%d", argv[1], processid);
	...
	ofstream fout;
	char logfile[32];
	sprintf(logfile, "mpbfs_%d_%d_%d.log", MAX_COROS, PROC_NUM, processid);
	fout.open(logfile);
	fout << "bfs " << duration << " s" << endl;
	fout.close();
	AccirrFinalize();
